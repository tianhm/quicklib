...
extern list <TThostFtdcInstrumentIDTypeStruct>::iterator tickname_Iter;
...
extern list <CThostFtdcRspUserLoginField>::iterator loginfaile_Iter;
...
extern list <CThostFtdcSpecificInstrumentField>::iterator subMarket_Iter;

extern list <CThostFtdcSpecificInstrumentField> unsubMarketlist;
extern list <CThostFtdcSpecificInstrumentField>::iterator unsubMarket_Iter;

extern list <CThostFtdcForQuoteRspField> forquotelist;
extern list <CThostFtdcForQuoteRspField>::iterator forquote_Iter;
...
void UpdatePriceHash(const char * InstrumentID, CThostFtdcDepthMarketDataField *pDepthMarketData)
{
	...
	*/
	...
	if (it != mapData.end())
	{
		...
		//printf("@@@ [%0.02f] [%0.02f] [%0.02f]\n", q->tick_data[4],q->M1_close[0], q->M1_high[0]);
		//printf("@@@ [%0.02f]  \n", q->tick_data[4] );
	}
...
void CMdSpi::OnFrontDisconnected(int nReason)
{
	...
	//tn.cmd = SYSTEM_NETCONNECT_BREAK;
	//EnterCriticalSection(&g_csdata);
	//cmdlist.push_back(tn);
	//LeaveCriticalSection(&g_csdata);
	//LeaveCriticalSection(&g_csdata);
...
void CMdSpi::OnFrontConnected()
{
	...
	//tn.cmd = SYSTEM_NETCONNECT_SCUESS;
	//EnterCriticalSection(&g_csdata);
	//cmdlist.push_back(tn);
	//LeaveCriticalSection(&g_csdata);
...
int CMdSpi::ReqUserLogin()
{
	...
	int iResult = mpUserApi->ReqUserLogin(&req, ++RequestID);
	//std::cout << "--> 发送用户登录请求: " << ((iResult == 0) ? "成功" : "失败") << std::endl;

	if (iResult != 0)
		cerr << "Failer: 发送登录行情请求 : " << ((iResult == 0) ? "成功" : "失败(") << iResult << ")" << endl;
	else
		cerr << "Scuess: 发送登录行情请求 : 成功" << endl;
...
int CMdSpi::ReqUserLogout()
{
	...
	strcpy(req.UserID, gUserID.c_str());
	//strcpy(req.Password, gPassword.c_str());
	int iResult = mpUserApi->ReqUserLogout(&req, ++RequestID);
	//std::cout << "--> 发送用户退出登录请求: " << ((iResult == 0) ? "成功" : "失败") << std::endl;

	if (iResult != 0)
		cerr << "Failer: 发送用户退出登录请求 : " << ((iResult == 0) ? "成功" : "失败(") << iResult << ")" << endl;
	else
		cerr << "Scuess: 发送用户退出登录请求 : 成功" << endl;
...
void CMdSpi::OnRspUserLogin(CThostFtdcRspUserLoginField *pRspUserLogin,CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	...
	{
		///获取当前交易日
		std::cout << "获取当前交易日 = " << mpUserApi->GetTradingDay() << std::endl;
		mInitOK = TRUE;
		...
		else
		{
			...
			//SubscribeMarketData("600200", 1);

			//UnSubscribeMarketData();
 

			//for (int id=0;id<stocknum;id++)
			//{
			//printf("[%d]%s\n",id, ppInstrumentID[id]);
...
void CMdSpi::SubscribeForQuoteRsp(char * InstrumentID)
{
	...
	if (iResult != 0)
	    ...
	    std::cout << "Scuess:询价[" << InstrumentID << "]成功" << std::endl;
}
...
void CMdSpi::SubscribeMarketData()
{
	...
	if (iResult != 0)
		...
		std::cout << "Scuess:订阅[" << ppInstrumentID[amount - 1] << "]行情成功" << std::endl;
...
//#include "Hash_Instrument.h"
//extern CCX_HASH  * CxHashList;
...
#include <windows.h>
using namespace std;
//extern typedef map<string, InstrumentInfo> QS_Strategy_Map2;
//extern QS_Strategy_Map2 * mapInstrument;// = new QS_Strategy_Map2;




extern void TestTransferDict();
extern void strategy1();
