...
def main():
    ...
    while(1):  #死循环，反复执行
        ...
        if (IsStockTrade()): 
            ...
            if market.LastPrice('zn1705')-market.LastPrice('zn1701')>300:
            #                                品种代码    多空方向    开仓还是平仓  市价或现价   价格  下单数量
            #下单函数原型 InsertOrder(self, instrumentID, direction,  offsetFlag, priceType,  price,   num):              
                OrderRef1 = trader.InsertOrder('zn1705', QL_D_Sell, QL_OF_Open, QL_OPT_LimitPrice, market.LastPrice('zn1705')-10, 1)
                time.sleep(0.5)
                #撤销所有未成交的委托单
                ret = trader.DeleteOrder('zn1705', OrderRef1) 
                        
                OrderRef2 = trader.InsertOrder('zn1701', QL_D_Buy, QL_OF_Open, QL_OPT_LimitPrice, market.LastPrice('zn1701')+10, 1)        
                time.sleep(1)
                #撤销所有未成交的委托单
                ret = trader.DeleteOrder('zn1701', OrderRef2)
